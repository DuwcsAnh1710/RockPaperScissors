    ...
    useEffect(() => {
        ...
        socket.on(
            ...
            ({
                ...
                // Set opponent choice for display
                const opponentId = Object.keys(choices).find(
                    id => id !== socket.id
                );
                if (opponentId) {
                    setOpponentChoice(choices[opponentId].choice);
                }
            }
        );
            ...
            <div className='game-header'>
                ...
                <div className='score-board'>
                    <div className='player-score'>
                        <span>
                            {roomPlayers[0]?.name}: {scores.player1}
                        </span>
                    </div>
                    <div className='vs-divider'>VS</div>
                    <div className='player-score'>
                        <span>
                            {roomPlayers[1]?.name}: {scores.player2}
                        </span>
                    </div>
                </div>
                {!gameOver && <p>🎯 Đầu tiên đạt 3 điểm sẽ thắng!</p>}
            </div>

            {/* Game Display */}
            <div className='choices-horizontal'>
                <div className='player-section'>
                    <div className='choice-label'>Bạn</div>
                ...
                <div className='opponent-section'>
                    <div className='choice-label'>Đối thủ</div>
                    <div className='choice-circle-large'>
                        {getChoiceDisplay(opponentChoice)}
                    </div>
                    <div className='choice-name'>
                        {opponentChoice
                            ? choices.find(c => c.value === opponentChoice)
?.name
                            : 'Chờ chọn'}
                    </div>
                </div>
